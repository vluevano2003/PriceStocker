...
public class ProductoView extends BaseDirectorioView<Producto> {
    ...
    /**
     * Sobrescribe el método show para asegurarse de que el servicio de moneda esté
     * inicializado antes de cargar la vista
     */
    @Override
    ...
    /**
     * Configura las columnas de la tabla de productos, incluyendo una columna de
     * acciones con botones para editar y eliminar cada producto
     */
    @Override
    protected void configurarColumnasTabla() {
        ...
        TableColumn<Producto, String> colId = UIFactory.crearColumna(idioma.get("product.col.id"),
                d -> String.valueOf(d.getIdProducto()), 50);
        colId.setMaxWidth(50);
        colId.setStyle("-fx-alignment: CENTER;");

        TableColumn<Producto, String> colNombre = UIFactory.crearColumna(idioma.get("product.col.product"),
                Producto::getNombreProducto, 150);
        ...
        TableColumn<Producto, String> colCategoria = UIFactory.crearColumna(idioma.get("product.col.category"), d -> {
            String cats = d.getCategorias().stream().map(Categoria::getNombreCategoria)
        ...
        colAcciones.setCellFactory(param -> new TableCell<>() {
            ...
            private final Button btnEliminar = UIFactory
                    .crearBotonTablaEliminar(() -> eliminarEntidad(getTableView().getItems().get(getIndex())));
    ...
    /**
     * Construye el formulario de creación/edición de productos, con pestañas para
     * detalles generales, proveedores, clientes, fabricantes, empresas y servicios
     * relacionados
     */
    @Override
    ...
    /**
     * Limpia los campos específicos del formulario de producto, restableciendo los
     * valores a sus estados iniciales y limpiando las listas de relaciones
     */
    @Override
    ...
    /**
     * Valida los campos del formulario y construye un objeto Producto con los datos
     * ingresados, incluyendo las relaciones con categorías, proveedores, clientes,
     * fabricantes, empresas y servicios. Luego llama al servicio para guardar el
     * producto y muestra una alerta con el resultado de la operación
     */
    @Override
    protected void registrarEntidad() {
        if (txtNombre.getText().trim().isEmpty()) {
            dialogService.mostrarAlerta(Alert.AlertType.WARNING, idioma.get("product.msg.validation.title"),
                    idioma.get("product.msg.req_name"), stage);
            return;
        }
        if (categoriasSeleccionadas.isEmpty()) {
            dialogService.mostrarAlerta(Alert.AlertType.WARNING, idioma.get("product.msg.validation.title"),
                    idioma.get("product.msg.req_category"), stage);
            return;
        }
        if (txtPrecio.getText().trim().isEmpty()) {
            dialogService.mostrarAlerta(Alert.AlertType.WARNING, idioma.get("product.msg.validation.title"),
                    idioma.get("product.msg.req_price"), stage);
            return;
        }
        ...
        try {
            ...
            dialogService.mostrarAlerta(Alert.AlertType.ERROR, idioma.get("product.msg.error.title"),
                    idioma.get("product.msg.numeric_error"), stage);
            return;
        }
        ...
        if (resultado.contains("exitosamente") || resultado.contains("guardad") || resultado.contains("successfully")
                || resultado.contains("saved")) {
            dialogService.mostrarAlerta(Alert.AlertType.INFORMATION, idioma.get("product.msg.success.title"), resultado,
                    stage);
            limpiarFormulario();
            cargarDatos();
        } else {
    ...
    /**
     * Prepara el formulario para editar un producto existente, cargando todos los
     * datos del producto seleccionado, incluyendo sus relaciones, y actualizando el
     * título y el texto del botón de guardar para reflejar que se está editando un
     * producto en lugar de creando uno nuevo
     * 
     * @param pResumen
     */
    ...
    /**
     * Elimina un producto, mostrando una confirmación antes de proceder. Si el
     * producto que se está eliminando es el mismo que se está editando actualmente,
     * limpia el formulario para evitar inconsistencias. Luego llama al servicio
     * para eliminar el producto y muestra una alerta con el resultado de la
     * operación
     * 
     * @param p
     */
    private void eliminarEntidad(Producto p) {
        ...
        if (dialogService.mostrarConfirmacion(idioma.get("product.msg.delete.title"),
                idioma.get("product.msg.delete.confirm", p.getNombreProducto()), stage)) {
            if (productoService.eliminarProducto(p)) {
                cargarDatos();
            } else {
                dialogService.mostrarAlerta(Alert.AlertType.ERROR, idioma.get("product.msg.error.title"),
                        idioma.get("product.msg.delete.error"), stage);
            }
        }
    }

    /**
     * Muestra un diálogo con los detalles completos de un producto, incluyendo sus
     * relaciones con proveedores, clientes, fabricantes, empresas y servicios. El
     * diálogo tiene una barra de navegación para cambiar entre las diferentes
    ...
    protected void mostrarDetalle(Producto p) {
        ...
        Label lblSub = new Label(
                (pFull.getFichaProducto() != null && !pFull.getFichaProducto().isEmpty()) ? pFull.getFichaProducto()
                        : idioma.get("product.detail.no_desc"));
        ...
        Node viewProv = UIFactory.crearTablaRelacion(FXCollections.observableArrayList(pFull.getProductoProveedores()),
                idioma.get("product.rel.supplier"), d -> d.getProveedor().getNombreProv(),
                idioma.get("product.rel.cost_purchase"), d -> formatearPrecioInteligente(d.getCosto(), d.getMoneda()),
                false);
        ajustarTablaFullWidth(viewProv);
        crearBotonPopup("BTN_PROV", idioma.get("product.tab.suppliers"), viewProv, navBar, dynamicContent,
                localNavButtons);

        Node viewCli = UIFactory.crearTablaRelacion(FXCollections.observableArrayList(pFull.getProductoClientes()),
                idioma.get("product.rel.client"), d -> d.getCliente().getNombreCliente(),
                idioma.get("product.rel.price_sale"), d -> formatearPrecioInteligente(d.getCosto(), d.getMoneda()),
                false);
        ...
        Node viewFab = UIFactory.crearTablaRelacion(FXCollections.observableArrayList(pFull.getProductoFabricantes()),
                idioma.get("product.rel.manufacturer"), d -> d.getFabricante().getNombreFabricante(),
                idioma.get("product.rel.cost_purchase"), d -> formatearPrecioInteligente(d.getCosto(), d.getMoneda()),
                false);
        ajustarTablaFullWidth(viewFab);
        crearBotonPopup("BTN_FAB", idioma.get("product.tab.manufacturers"), viewFab, navBar, dynamicContent,
                localNavButtons);

        Node viewEmp = UIFactory.crearTablaRelacion(FXCollections.observableArrayList(pFull.getProductoEmpresas()),
                idioma.get("product.rel.company"), d -> d.getEmpresa().getNombreEmpresa(),
                idioma.get("product.rel.cost_market"), d -> formatearPrecioInteligente(d.getCosto(), d.getMoneda()),
                false);
        ajustarTablaFullWidth(viewEmp);
        crearBotonPopup("BTN_EMP", idioma.get("product.tab.companies"), viewEmp, navBar, dynamicContent,
                localNavButtons);

        Node viewServ = UIFactory.crearTablaRelacion(FXCollections.observableArrayList(pFull.getServicios()),
                idioma.get("product.rel.service"),
                s -> s.getDescripcionServicio()
                        + (s.getPrestador() != null ? " (" + s.getPrestador().getNombrePrestador() + ")" : ""),
                idioma.get("product.lbl.base_price"),
                s -> formatearPrecioInteligente(s.getCostoServicio(), s.getMonedaServicio()), false);
        ajustarTablaFullWidth(viewServ);
        crearBotonPopup("BTN_SERV", idioma.get("product.tab.linked_services"), viewServ, navBar, dynamicContent,
                localNavButtons);
    ...
    /**
     * Inicializa los campos de entrada del formulario general del producto,
     * incluyendo validaciones para los campos numéricos y un cálculo dinámico de
     * equivalencia de moneda basado en la selección del usuario. También configura
     * un listener para actualizar la conversión cada vez que el usuario cambia el
     * precio o la moneda seleccionada
     */
    ...
    /**
     * Calcula la equivalencia del precio ingresado en el campo de precio general,
     * convirtiendo entre MXN y USD según la moneda seleccionada y la moneda
     * preferida del sistema. Actualiza el label de conversión para mostrar el valor
     * equivalente en la otra moneda, utilizando el tipo de cambio obtenido del
     * servicio de moneda. Si el campo de precio está vacío o contiene un valor no
     * numérico, limpia el label de conversión
     * 
     * @param inputMonto
     * @param comboMoneda
     * @param labelDestino
     ...
     * sistema, utilizando el tipo de cambio actual. Si la moneda del precio ya es
     * la misma que la moneda preferida, simplemente muestra el precio original sin
     * conversión. Si ocurre algún error al obtener el tipo de cambio o realizar la
     * conversión, devuelve el texto original sin formato adicional
     * 
     * @param precio
     * @param monedaItem
     * @return
    ...
    /**
     * Construye la vista general del formulario de producto, organizando los campos
     * de entrada para el nombre, ficha técnica, alterno, precio y existencia, junto
     * con un selector de categorías. Aplica estilos para resaltar los títulos de
     * cada sección y asegurar una presentación clara y organizada de la información
    ...
    /**
     * Crea un panel para gestionar las relaciones entre el producto y otras
     * entidades (proveedores, clientes, fabricantes, empresas), permitiendo al
     * usuario seleccionar un ítem disponible, ingresar un costo o precio asociado,
     * y agregarlo a la lista de relaciones del producto. El panel también muestra
     * una tabla con las relaciones ya agregadas, y maneja el caso en que no haya
     * ítems disponibles para relacionar mostrando un estado vacío informativo
     * 
     * @param <T>
     * @param <R>
    ...
    private <T, R> VBox crearPanelGestionRelacion(List<T> itemsDisponibles, ObservableList<R> itemsAgregados,
        ...
        if (itemsDisponibles.isEmpty()) {
            ...
            emptyState.setStyle(
                    "-fx-background-color: #F9FAFB; -fx-border-color: #E5E7EB; -fx-border-radius: 6; -fx-border-style: dashed;");
            ...
            txtCosto.setTextFormatter(new TextFormatter<>(
                    change -> change.getControlNewText().matches("\\d*|\\d+\\.\\d*") ? change : null));
            ...
            btnAdd.setOnAction(e -> {
                ...
                if (itemSel != null && !costoTxt.isEmpty()) {
                    boolean yaExiste = itemsAgregados.stream().anyMatch(rel -> {
                        return textExtractor.apply(itemSel).equals(col1Extractor.apply(rel));
                    });

                    if (yaExiste) {
                        dialogService.mostrarAlerta(
                                Alert.AlertType.WARNING,
                                idioma.get("product.msg.duplicate.title"),
                                idioma.get("product.msg.duplicate.content"),
                                stage);
                        return;
                    }
        ...
        root.getChildren().add(UIFactory.crearTablaRelacion(itemsAgregados, labelItem, col1Extractor, labelCosto,
                col2Extractor, true));
        return root;
    }

    /**
     * Crea un panel con información detallada de un producto, mostrando campos
     * fijos como el código alterno, existencia, precio base y categorías asociadas.
     * Aplica estilos para resaltar los valores importantes como el precio y la
     * existencia, utilizando colores para indicar si el producto está en stock o
     * agotado. También maneja el caso en que no haya información disponible para
     * ciertos campos mostrando un texto predeterminado
     * 
     * @param p
     * @return
     */
    private GridPane crearPanelDetallesFijos(Producto p) {
        ...
        VBox vAlterno = UIFactory.crearDatoDetalle(idioma.get("product.detail.altern"),
    ...
    /**
     * Crea un botón de navegación para las pestañas del formulario de producto,
     * configurando su estilo y su acción para mostrar la vista correspondiente en
     * el panel de contenido. También mantiene un mapa de botones para actualizar su
     * estilo cuando se selecciona una pestaña, resaltando la pestaña activa y
     * aplicando un estilo diferente a las pestañas inactivas
     * 
     * @param id
     * @param titulo
     * @param container
     ...
     * activo y aplicar un estilo diferente a los botones inactivos. El botón activo
     * tendrá un fondo resaltado, texto del color primario y un peso de fuente más
     * alto, mientras que los botones inactivos tendrán un fondo transparente, texto
     * gris y un efecto hover para mejorar la experiencia del usuario
     * 
     * @param activo
     */
    ...
    /**
     * Crea un botón para las vistas de detalle del producto, configurando su acción
     * para mostrar la vista correspondiente en el panel de contenido y actualizando
     * el estilo de los botones para resaltar el botón activo. Este método es
     * similar a crearBotonNav pero se utiliza específicamente para las vistas de
     * detalle que se muestran en un diálogo modal, manteniendo un mapa local de
     * botones para manejar el estilo dentro del contexto del diálogo
     * 
     * @param id
     * @param titulo
     * @param view
     ...
     * @param mapButtons
     */
    private void crearBotonPopup(String id, String titulo, Node view, HBox container, BorderPane contentArea,
    ...
    /**
     * Crea un panel para seleccionar y gestionar las categorías asociadas a un
     * producto, permitiendo al usuario elegir entre las categorías existentes o
     * crear una nueva categoría directamente desde el formulario. El panel muestra
     * una lista de categorías seleccionadas con la opción de eliminar cada
     * categoría individualmente, y maneja la lógica para evitar duplicados tanto en
     * las categorías existentes como en las nuevas categorías creadas por el
     * usuario
     * 
     * @return
     */
    ...
    /**
     * Crea un panel para gestionar los servicios relacionados con el producto,
     * permitiendo al usuario seleccionar un servicio existente, ingresar un costo
     * base para ese servicio y agregarlo a la lista de servicios relacionados del
     * producto. El panel también muestra una tabla con los servicios ya
     * relacionados, incluyendo su descripción y costo base, y maneja el caso en que
     * no haya servicios disponibles para relacionar mostrando un estado vacío
     * informativo
     * 
     * @return
     */
    private VBox crearSubFormularioServicios() {
        ...
        if (lista.isEmpty()) {
            ...
            box.getChildren().addAll(UIFactory.crearGrupoInput(idioma.get("product.rel.service"), cmb), groupCosto,
                    btnAdd);
        }
     ...
     * de texto personalizado. Este método establece un StringConverter para
     * convertir los objetos a su representación de texto en el ComboBox, y también
     * configura una celda personalizada para mostrar el texto extraído en la lista
     * desplegable y en el botón del ComboBox. Esto permite que el ComboBox muestre
     * información relevante de los objetos sin necesidad de que los objetos tengan
     * un método toString() adecuado
     * 
     * @param <T>
     * @param combo
     * @param textExtractor
    ...
    /**
     * Ajusta el ancho de las columnas de una TableView para que ocupen todo el
     * espacio disponible, distribuyendo el ancho de manera equitativa entre las
     * columnas. Este método se aplica recursivamente a cualquier TableView
     * encontrada dentro del nodo proporcionado, lo que permite que las tablas
     * dentro de los paneles de detalle del producto se ajusten correctamente al
     * tamaño del diálogo modal
     * 
     * @param view
     */
