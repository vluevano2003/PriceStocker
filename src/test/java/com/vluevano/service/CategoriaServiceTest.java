...
class CategoriaServiceTest {
    ...
    @Test
    @DisplayName("Debe retornar la lista de categorías del repositorio")
    void testConsultarCategorias() {
        // Simulamos que la BD tiene 2 categorías
        when(categoriaRepository.findAll()).thenReturn(Arrays.asList(new Categoria(), new Categoria()));

        List<Categoria> resultado = categoriaService.consultarCategorias();

        assertEquals(2, resultado.size(), "Debería retornar 2 categorías");
        verify(categoriaRepository, times(1)).findAll(); // Verificamos que llamó al repo
    }
}